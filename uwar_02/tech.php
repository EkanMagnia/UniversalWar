...
{
	$completeRequest = mysql_query("SELECT complete FROM uwar_constructions WHERE userid='$Userid' AND constructionid='$constructionid'"); 
	$completeVal = mysql_fetch_array($completeRequest);
	$completeVal = $completeVal["complete"];

	//If there is any research in progress, select it
	$otherinprogress = "false";
	$inprogress = mysql_query("SELECT * FROM uwar_constructions WHERE userid='$Userid' AND activated=1 AND eta > 0");
	if(mysql_num_rows($inprogress) > 0)
	{
		while( $conprogress = mysql_fetch_array($inprogress) )
		{
			...
			if($contype["Type"] == "r")	$otherinprogress = "true";
		}
	}

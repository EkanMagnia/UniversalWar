...
while ($user = mysql_fetch_array($request))
{
    ...
    if (mysql_num_rows($target_request) > 0)
    {
        ...
        <?
		$total_attackers_ships = 0;
		$total_defenders_ships = 0;
        while ($target = mysql_fetch_array($target_request))
        {
            ...
            $shipsSQL = mysql_query("SELECT * FROM uwar_fships WHERE userid='$target[userid]' and fleetnum='$target[fleetnum]' ORDER by shipid",$db) or die(mysql_error());
				...
				if ($target["action"] == "a")
					$total_attackers_ships += $ShipsTotal;
				else 
					$total_defenders_ships += $ShipsTotal;
            ?><tr>
        ...
        ?><td colspan=4 bgcolor="<?=$tdbg2;?>"><center>Totals : <font color="#FF0000"><?print number_format($total_attackers_ships);?></font> / <font color="#339900"><?=number_format($total_defenders_ships);?></td>
...
<?
$userSQL = mysql_query("SELECT * FROM uwar_users WHERE sysid='$myrow[sysid]'",$db);
...
if ($outgoings != TRUE) 
{
    ?><td colspan="5" bgcolor="<?=$tdbg2;?>"><center>There are no outgoing fleets from our system.</td><?
}

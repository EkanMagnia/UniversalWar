...
if ( !isset( $action ) ) {
headerDsp( "Topics" );
$request = mysql_query("SELECT * FROM uwar_bugsforum WHERE replyid=0 ORDER BY timereply DESC", $db);
...
?>
<tr>
<td bgcolor="<?=$tdbg2;?>" colspan="4"><center>There are currently no bugs reports.</center></td>
<?
}
else {
...
?>
<tr>
<td bgcolor="<?=$tdbg2;?>"><a href="<? print $PHP_SELF; ?>?action=viewtopic&topicid=<? print $topic['id']; ?>"><? print $topic['subject']; ?></a><? if ( $topic['userid'] == $Userid) {?> (<a href="<? print $PHP_SELF; ?>?action=remove&postid=<? print $topic['id']; ?>">Remove</a>)<?}?></td>
...
<?
$replySQL = mysql_query("SELECT id FROM uwar_bugsforum WHERE replyid=$topic[id]", $db);
...
<tr>
<td>&nbsp;</td>
<td><input type="submit" value="Report Bug"></td>
</table>
</form>
</center>
<?
...
$time = time();
mysql_query("INSERT INTO uwar_bugsforum ( id, userid, time, subject, content, threadstarter, timereply, replyid, nick ) values ( '', $Userid, $time, '$subject', '$content', $Userid, $time, 0, '$myrow[nick])",$db);
?>
...
<center><b>Topic Posted</b><br>If you are not automatically forwarded to the bugs report page, <a href="<? print $PHP_SELF; ?>">click here</a></center><br/>
<?
}
else {
?>
<center>You cannot post nothing!<br><a href="<? print $PHP_SELF; ?>">Click here</a> to return to the bugs report page.</center><br/>
<?
}
}
...
<?
$request = mysql_query("SELECT * FROM uwar_bugsforum WHERE id = $topicid ",$db);
$request2 = mysql_query("SELECT * FROM uwar_bugsforum WHERE replyid = $topicid ORDER by time", $db);

$topic = mysql_fetch_array( $request );
$request3 = mysql_query("SELECT nick FROM uwar_users WHERE id = $topic[userid]",$db);
...
$content = str_replace( "'", "`", $content );
$time = time();
mysql_query("INSERT INTO uwar_bugsforum ( id, userid, time, subject, content, threadstarter, timereply, replyid, nick ) values ( '', $Userid, $time, '$subject', '$content', 0, 0, $topicid, '$myrow[nick]' )",$db);
mysql_query("UPDATE uwar_bugsforum SET timereply = $time WHERE id = $topicid",$db);
?>
<script language="Javascript">
function moveNow() {
...
$request = mysql_query("SELECT * FROM uwar_bugsforum WHERE id = $postid",$db);
...
mysql_query("DELETE FROM uwar_bugsforum WHERE id = $postid",$db);
mysql_query("DELETE FROM uwar_bugsforum WHERE replyid = $postid",$db);
?>
<script language="Javascript">
function moveNow() {
parent.location = '<? print $PHP_SELF; ?>';
...
<center><b>Post Removed</b><br>If you are not automatically forwarded to the bugs report page, <a href="<? print $PHP_SELF; ?>">click here</a></center><br/>
<?
}
}
else {
?>
<center>You are not authorized to remove this post<br><a href="<? print $PHP_SELF; ?>">Click here</a> to return to the bugs report page</center><br/>
<?
}
}
...
mysql_query("UPDATE uwar_bugsforum SET subject='$subject', content='$content' WHERE id = $postid",$db);
?>
<script language="Javascript">
function moveNow() {
parent.location = '<? print $PHP_SELF; ?>';
...
<center><b>Changes Saved</b><br>If you are not automatically forwarded to the bugs report page, <a href="<? print $PHP_SELF; ?>">click here</a></center><br/>
<?
}
else {
?>

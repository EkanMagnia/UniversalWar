...
if ( !isset( $action ) ) {
headerDsp( "Topics" );
$request = mysql_query("SELECT * FROM uwar_sgovernment WHERE sysid=$myrow[sysid] && replyid=0 ORDER BY timereply DESC", $db);
...
?>
<tr>
<td bgcolor="<?=$tdbg2;?>" colspan="4"><center>There are currently no topics from your system</center></td>
<?
}
else {
...
?>
<tr>
<td bgcolor="<?=$tdbg2;?>"><a href="<? print $PHP_SELF; ?>?action=viewtopic&topicid=<? print $topic['id']; ?>"><? print $topic['subject']; ?></a><? if ( $topic['userid'] == $Userid || $myrow['commander'] == 3 || $myrow['commander'] == 1) {?> (<a href="<? print $PHP_SELF; ?>?action=remove&postid=<? print $topic['id']; ?>">Remove</a>)<?}?></td>
...
<?
$replySQL = mysql_query("SELECT id FROM uwar_sgovernment WHERE sysid=$myrow[sysid] && replyid=$topic[id]", $db);
...
$time = time();
mysql_query("INSERT INTO ".$Uwar["sgovernment"]." ( id, userid, time, subject, content, threadstarter, sysid, timereply, replyid ) values ( '', $Userid, $time, '$subject', '$content', $Userid, $myrow[sysid], $time, 0 )",$db);
?>
...
<center><b>Topic Posted</b><br>If you are not automatically forwarded to the government page, <a href="<? print $PHP_SELF; ?>">click here</a></center><br/>
<?
}
else {
?>
<center>You cannot post anything!<br><a href="<? print $PHP_SELF; ?>">Click here</a> to return to the government topics</center><br/>
<?
}
}
...
<?
$request = mysql_query("SELECT * FROM ".$Uwar["sgovernment"]." WHERE id = $topicid AND sysid=$myrow[sysid]",$db);
$request2 = mysql_query("SELECT * FROM ".$Uwar["sgovernment"]." WHERE replyid = $topicid AND sysid=$myrow[sysid] ORDER by time", $db);
...
if ($topic["sysid"] == $myrow["sysid"])
{
	headerDsp( $topic['subject']." - Posted by".$TopicUsr['nick']." - ".date("M j, Y H:i:s",$topic['time']) );
	?>
	<br><br>
	<center>
	<table border="0" cellpadding="4" cellspacing="1" width="90%">
...
headerDsp( $reply['subject']." - Posted by".$ReplyUsr['nick']." - ".date("M j, Y H:i:s",$reply['time']) );
?>
<br><br>
<center>
<table border="0" cellpadding="4" cellspacing="1" width="90%">
...
$content = str_replace( "'", "`", $content );
$time = time();
mysql_query("INSERT INTO ".$Uwar["sgovernment"]." ( id, userid, time, subject, content, threadstarter, sysid, timereply, replyid ) values ( '', $Userid, $time, '$subject', '$content', 0, $myrow[sysid], 0, $topicid )",$db);
mysql_query("UPDATE ".$Uwar["sgovernment"]." SET timereply = $time WHERE id = $topicid",$db);
?>
<script language="Javascript">
function moveNow() {
...
$request = mysql_query("SELECT * FROM ".$Uwar["sgovernment"]." WHERE id = $postid",$db);
$post = mysql_fetch_array( $request );

if ( ($post['userid'] == $Userid) || (($myrow['commander'] == 3 || $myrow['commander'] == 1) && $replyid == 0 ) ) {
if ( !isset( $confirmed ) ) {
?>
<center>
...
mysql_query("DELETE FROM ".$Uwar["sgovernment"]." WHERE id = $postid",$db);
mysql_query("DELETE FROM ".$Uwar["sgovernment"]." WHERE replyid = $postid",$db);
?>
<script language="Javascript">
function moveNow() {
parent.location = '<? print $PHP_SELF; ?>';
...
mysql_query("UPDATE ".$Uwar["sgovernment"]." SET subject='$subject', content='$content' WHERE id = $postid",$db);
?>
<script language="Javascript">
function moveNow() {
parent.location = '<? print $PHP_SELF; ?>';
...
<center><b>Changes Saved</b><br>If you are not automatically forwarded to the government page, <a href="<? print $PHP_SELF; ?>">click here</a></center><br/>
<?
}
else {
?>

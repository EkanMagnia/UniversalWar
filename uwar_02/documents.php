...
if ( !isset( $action ) ) {
headerDsp( "Topics" );
$request = mysql_query("SELECT * FROM uwar_agovernment WHERE tagid=$myrow[tagid] && replyid=0 ORDER BY timereply DESC", $db);
...
?>
<tr>
<td bgcolor="<?=$tdbg2;?>" colspan="4"><center>There are currently no topics from your alliance</center></td>
<?
}
else {
...
?>
<tr>
<td bgcolor="<?=$tdbg2;?>"><a href="<? print $PHP_SELF; ?>?action=viewtopic&topicid=<? print $topic['id']; ?>"><? print $topic['subject']; ?></a><? if ( $topic['userid'] == $Userid || $myrow['leader'] == 3 ) {?> (<a href="<? print $PHP_SELF; ?>?action=remove&postid=<? print $topic['id']; ?>">Remove</a>)<?}?></td>
...
<?
$replySQL = mysql_query("SELECT id FROM uwar_agovernment WHERE tagid=$myrow[tagid] && replyid=$topic[id]", $db);
...
$time = time();
mysql_query("INSERT INTO ".$Uwar["agovernment"]." ( id, userid, time, subject, content, threadstarter, tagid, timereply, replyid ) values ( '', $Userid, $time, '$subject', '$content', $Userid, $myrow[tagid], $time, 0 )",$db);
?>
...
<center><b>Topic Posted</b><br>If you are not automatically forwarded to the documents page, <a href="<? print $PHP_SELF; ?>">click here</a></center><br/>
<?
}
else {
?>
<center>You cannot post anything!<br><a href="<? print $PHP_SELF; ?>">Click here</a> to return to the documents topics</center><br/>
<?
}
}
...
<?
$request = mysql_query("SELECT * FROM ".$Uwar["agovernment"]." WHERE id = $topicid AND tagid=$myrow[tagid] ORDER by time",$db);
$request2 = mysql_query("SELECT * FROM ".$Uwar["agovernment"]." WHERE replyid = $topicid AND tagid=$myrow[tagid] ORDER by time AND timereply", $db);
...
$content = str_replace( "'", "`", $content );
$time = time();
mysql_query("INSERT INTO ".$Uwar["agovernment"]." ( id, userid, time, subject, content, threadstarter, tagid, timereply, replyid ) values ( '', $Userid, $time, '$subject', '$content', 0, $myrow[tagid], 0, $topicid )",$db);
mysql_query("UPDATE ".$Uwar["agovernment"]." SET timereply = $time WHERE id = $topicid",$db);
?>
<script language="Javascript">
function moveNow() {
...
?>
<center>You cannot post anything!<br><a href="<? print $PHP_SELF; ?>?action=viewtopic&topicid=<? print $topicid; ?>">Click here</a> to return to the topic you were replying to</center><br/>
<?
}
}
elseif ( $action == "remove" && isset($postid) ) {
$request = mysql_query("SELECT * FROM ".$Uwar["agovernment"]." WHERE id = $postid",$db);
$post = mysql_fetch_array( $request );

if ( ($post['userid'] == $Userid) || ($myrow['leader'] == 3 && $replyid == 0 ) ) {
if ( !isset( $confirmed ) ) {
?>
<center>
...
mysql_query("DELETE FROM ".$Uwar["agovernment"]." WHERE id = $postid",$db);
mysql_query("DELETE FROM ".$Uwar["agovernment"]." WHERE replyid = $postid",$db);
?>
<script language="Javascript">
function moveNow() {
parent.location = '<? print $PHP_SELF; ?>';
...
<center><b>Post Removed</b><br>If you are not automatically forwarded to the documents page, <a href="<? print $PHP_SELF; ?>">click here</a></center><br/>
<?
}
}
else {
?>
<center>You are not authorized to remove this post<br><a href="<? print $PHP_SELF; ?>">Click here</a> to return to the documents page</center><br/>
<?
}
}
...
mysql_query("UPDATE ".$Uwar["agovernment"]." SET subject='$subject', content='$content' WHERE id = $postid",$db);
?>
<script language="Javascript">
function moveNow() {
parent.location = '<? print $PHP_SELF; ?>';
...
<center><b>Changes Saved</b><br>If you are not automatically forwarded to the documents page, <a href="<? print $PHP_SELF; ?>">click here</a></center><br/>
<?
}
else {
?>
